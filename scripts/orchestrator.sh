...
#     way that thrashes the Pi's USB bus or the SSD. A shared in-process lock
...
: "${LOG_LEVEL:=INFO}"
: "${SSD_USAGE_WARN_PCT:=85}"            # warn threshold for the disk guard
: "${SSD_USAGE_HALT_PCT:=92}"            # above this, skip snapshots to protect disk
...
snapshot_loop() {
  ...
  while true; do
    ...
    used=$(disk_used_pct)
    if [ "${used:-0}" -ge "${SSD_USAGE_HALT_PCT}" ]; then
      log snapshot "DISK GUARD: /data at ${used}% >= halt ${SSD_USAGE_HALT_PCT}%; SKIPPING snapshot to protect disk. Tighten retention!"
      continue
    fi
    ...
    used=$(disk_used_pct)
    if [ "${used:-0}" -ge "${SSD_USAGE_WARN_PCT}" ]; then
      log snapshot "DISK WARNING: /data at ${used}% >= warn ${SSD_USAGE_WARN_PCT}%. Consider tightening KEEP_* retention."
    fi
  done
}
